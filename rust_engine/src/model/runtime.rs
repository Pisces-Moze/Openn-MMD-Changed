    ...
    pub fn batch_get_sub_mesh_data(&self, output: &mut [u8]) -> usize {
        ...
        for (i, submesh) in self.submeshes.iter().enumerate() {
            ...
            let alpha = self
                ...
                .map(|m| m.diffuse.w)
                .unwrap_or(1.0f32);
            // A PMX may use one material for both its head and body (common for
            // VRChat/FBX conversions). Material-only masking cannot hide the
            // local head without hiding the whole avatar, so honor the
            // existing geometric head classification per submesh as well.
            let hidden_local_head = self.first_person_enabled
                && self.head_submesh_flags.get(i).copied().unwrap_or(false);
