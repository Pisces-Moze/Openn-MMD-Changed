...
impl BoneSet {
    ...
    pub fn build_hierarchy(&mut self) {
        ...
        self.sorted_indices.sort_by(|&a, &b| {
            self.links[a]
                .transform_level
                .cmp(&self.links[b].transform_level)
        });
    ...
    pub fn links_with_name<'a>(
        &'a self,
        name: &'a str,
    ) -> impl Iterator<Item = (usize, &'a BoneLink)> {
        self.links
            .iter()
            .enumerate()
    ...
    pub fn update_transforms(&mut self, after_physics: bool) {
        ...
        for &idx in &self.sorted_indices.clone() {
            ...
            let needs_append =
                self.links[idx].is_append_rotate() || self.links[idx].is_append_translate();
