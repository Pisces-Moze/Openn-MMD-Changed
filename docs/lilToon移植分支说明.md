...
回退使用与 VRM SpringBone 共用的原生求解器，并接收实体平移/旋转产生的惯性；
`secondary_motion.json` 可以给非标准骨名显式声明衣服、头发、耳朵、尾巴、软组织
和装饰骨链及其参数。PMX 一旦含有刚体，配置即被忽略并使用标准 Bullet3，避免重复
求解。配置文件和模板的完整字段见通用开发手册 4.1 节。

三渲二基础层继续采用 MC-MMD-rust 的 ToonShader 光照分段、纹理/材质调制和受伤
变红通道；本分支只在其上增加逐材质 lilToon 阴影参数、独立发光、边缘光与 GPU
扩壳描边。因此切换材质配置不会绕过原版实体光照，关闭 lilToon 扩展后仍保留原
MC-MMD-rust 的卡通明暗效果。
