...
public final class FirstPersonManager {
    ...
    public static boolean shouldRenderFirstPerson() {
        ...
        // InventoryScreen renders the local player through the normal entity
        // renderer. Never reuse the native first-person material mask there,
        // otherwise the preview inherits the hidden head/accessory state.
        if (InventoryRenderHelper.isInventoryScreen()) return false;
        if (mc.player == null || !MmdSkinRendererPlayerHelper.isUsingMmdModel(mc.player)) return false;
        // The embedded Changed form has no model-selector/config screen; using
        // that fixed form is the opt-in equivalent of MC-MMD-rust's setting.
        return RuntimeConfigPortHolder.get().isFirstPersonModelEnabled()
                || net.ltxprogrammer.changed.process.ProcessTransfur.getPlayerTransfurVariantSafe(mc.player)
                .filter(instance -> instance.getChangedEntity() instanceof com.moze.openmmdchanged.entity.MmdLatexEntity)
                .isPresent();
    }
