...
public final class MmdSkinRendererPlayerHelper {
    ...
    public static boolean isUsingMmdModel(Player player) {
        ...
        if (net.ltxprogrammer.changed.process.ProcessTransfur.getPlayerTransfurVariantSafe(player)
                .filter(instance -> instance.getChangedEntity() instanceof MmdLatexEntity)
                .filter(instance -> instance.getTransfurProgression(Minecraft.getInstance().getPartialTick()) >= 1.0F)
                .isPresent()) {
            return true;
        }
