...
public class MmdSkinRenderer<T extends Entity> extends EntityRenderer<T> {
    ...
    public void render(T entityIn, float entityYaw, float tickDelta, PoseStack matrixStackIn,
        ...
        AbstractClientPlayer underlyingPlayer = entityIn instanceof ChangedEntity changed
                && changed.getUnderlyingPlayer() instanceof AbstractClientPlayer player ? player : null;
        Minecraft minecraft = Minecraft.getInstance();
        boolean localPlayer = underlyingPlayer != null && minecraft.player != null
                && minecraft.player.getUUID().equals(underlyingPlayer.getUUID());
        if (localPlayer) {
            FirstPersonManager.preRender(model.modelInstance().getModelHandle(), size[0], true);
        }
        boolean firstPerson = localPlayer && FirstPersonManager.isActive();
        ...
        if (InventoryRenderHelper.isInventoryScreen()) {
            ...
            if (underlyingPlayer != null) {
                ...
                ItemRenderHelper.renderItems(underlyingPlayer, model, matrixStackIn, bufferIn,
                        packedLightIn, 1.0F, tickDelta, size[0]);
            }
        }
    ...
    private void renderInInventory(T entityIn, ManagedModel model, float entityYaw,
        ...
        if (mc.gameMode != null && mc.gameMode.getPlayerMode() != GameType.CREATIVE
                && mc.screen instanceof InventoryScreen inventoryScreen) {
            posX = inventoryScreen.getRecipeBookComponent().updateScreenPosition(mc.screen.width, 176);
        ...
        reusableQuat.identity()
                ...
                .rotateY((float) Math.PI - entityIn.getYRot() * ((float) Math.PI / 180F));
