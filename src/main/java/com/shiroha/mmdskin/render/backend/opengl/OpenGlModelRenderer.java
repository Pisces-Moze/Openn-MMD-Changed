...
final class OpenGlModelRenderer {
    ...
    static void render(OpenGlModelInstance target, Entity entityIn, float entityYaw, float entityPitch,
        ...
        // A custom GLSL program cannot consume an arbitrary Iris shader pack's
        // shadow buffers. Keep Iris' currently bound entity/shadow program when
        // a pack is active so the model both casts and receives pack shadows.
    ...
    private static void renderStandard(OpenGlModelInstance target, Minecraft minecraft,
                                       LightingHelper.LightData light, PoseStack deliverStack, boolean hurt) {
        boolean irisActive = IrisCompat.isIrisShaderActive();
        float colorFactor = irisActive ? 1.0f : light.intensity();
        float hurtGreenBlue = hurt ? 0.35f : 1.0f;
        RenderSystem.setShaderColor(colorFactor, colorFactor * hurtGreenBlue,
                colorFactor * hurtGreenBlue, 1.0f);
        ...
        drawStandardSubMeshes(target, minecraft, colorFactor, hurtGreenBlue);
    ...
    private static void drawStandardSubMeshes(OpenGlModelInstance target, Minecraft minecraft,
                                              float colorFactor, float hurtGreenBlue) {
        int normalBlock = target.lastBlockBrightness;
        int normalSky = target.lastSkyBrightness;
        int missingTextureId = minecraft.getTextureManager()
                .getTexture(TextureManager.INTENTIONAL_MISSING_TEXTURE)
                .getId();
        SubMeshDrawHelper.draw(
                ...
                materialId -> {
                    ...
                    float lit = Math.max(colorFactor, material.unlitStrength());
                    RenderSystem.setShaderColor(lit, lit * hurtGreenBlue, lit * hurtGreenBlue, 1.0f);
                });
        uploadLightBufferIfNeeded(target, 240, 240);
        SubMeshDrawHelper.draw(
                target.subMeshDataBuf,
                target.subMeshCount,
                target.indexElementSize,
                ...
                materialId -> target.mats[materialId].isFacialFeature(),
                materialId -> {
                    float lit = Math.max(colorFactor, target.mats[materialId].unlitStrength());
                    RenderSystem.setShaderColor(lit, lit * hurtGreenBlue, lit * hurtGreenBlue, 1.0f);
                });
        uploadLightBufferIfNeeded(target, normalBlock, normalSky);
    }
    ...
    private static void renderFullbrightLayers(OpenGlModelInstance target) {
        ...
        shader.use(target.projMatBuff, target.modelViewMatBuff);
        int posLoc = shader.positionLocation();
        int uvLoc = shader.uvLocation();
        if (posLoc >= 0) {
    ...
    private static void drawToonSubMeshes(OpenGlModelInstance target, Minecraft minecraft, float lightIntensity) {
        int missingTextureId = minecraft.getTextureManager()
                .getTexture(TextureManager.INTENTIONAL_MISSING_TEXTURE)
                .getId();
        SubMeshDrawHelper.draw(
