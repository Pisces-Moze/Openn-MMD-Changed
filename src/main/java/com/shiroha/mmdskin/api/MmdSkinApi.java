...
public final class MmdSkinApi {
    ...
    /** Finds a PMX/PMD material by its exact exported name. */
    public static int findMaterialIndex(long modelHandle, String materialName) {
        if (modelHandle == 0 || materialName == null || materialName.isEmpty()) {
            return -1;
        }
        try {
            int materialCount = sanitizeCount("material", modelQueryPort.getMaterialCount(modelHandle), modelHandle);
            for (int index = 0; index < materialCount; index++) {
                if (materialName.equals(modelQueryPort.getMaterialName(modelHandle, index))) {
                    return index;
                }
            }
        } catch (Exception e) {
            logger.debug("findMaterialIndex failed for modelHandle={}", modelHandle, e);
        }
        return -1;
    }

    /** Changes one material's visibility on a loaded model instance. */
    public static boolean setMaterialVisible(long modelHandle, int materialIndex, boolean visible) {
        if (modelHandle == 0 || materialIndex < 0) {
            return false;
        }
        try {
            int materialCount = sanitizeCount("material", modelQueryPort.getMaterialCount(modelHandle), modelHandle);
            ...
            modelPort.setMaterialVisible(modelHandle, materialIndex, visible);
            return true;
        } catch (Exception e) {
            logger.debug("setMaterialVisible failed for modelHandle={}, material={}",
                    modelHandle, materialIndex, e);
            return false;
        }
    }
