...
void main() {
    ...
    finalColor = mix(finalColor, albedo, clamp(MaterialUnlit, 0.0, 1.0));
    ...
    float cyanBlue = smoothstep(0.12, 0.32, saturation)
            * smoothstep(albedo.r * 0.95, albedo.r * 1.25 + 0.03, max(albedo.g, albedo.b));
    float brightCoating = smoothstep(0.40, 0.68, maxChannel);
    finalColor += albedo * cyanBlue * brightCoating * EmissionStrength;
